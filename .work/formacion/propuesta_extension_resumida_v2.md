...
El programa trabaja esas tres preguntas con un marco conceptual (IEDD, basado en Spec-Driven Development), con dos implementaciones de referencia construidas específicamente para responderlas, y con la evidencia empírica de un experimento real que las pone a prueba a lo largo de un ciclo completo de desarrollo. El objetivo no es que los participantes adopten esas implementaciones: es que entiendan los principios que las hacen funcionar y puedan construir las suyas.
...
**Ejercicio práctico:** escribir un spec-kit completo para la historia de usuario modelada en el módulo anterior. Revisión en pares con criterio explícita de evaluación.
...
**Evidencia:** el overhead del flujo convergió a ~18 minutos por historia de usuario en el experimento AtaraxiaDive, sobre una arquitectura hexagonal de complejidad media, ejecutado por un desarrollador con experiencia en el método.
...
### Módulo 6 — Evidencia del ciclo completo y diseño del proceso propio (2 hs)

Enseñar un método sin mostrar sus datos es vender teoría. Este módulo cierra el programa con la evidencia del experimento en su ciclo completo —desde la primera especificación hasta el prototipo desplegado— y trabaja con el equipo en el diseño concreto de su propio proceso.

**Contenidos:**
- Qué confirma el experimento AtaraxiaDive en el ciclo completo (SP1–SP5): la especificación formal reduce la inconsistencia entre lo especificado y lo implementado; el flujo agentic es gobernable con puntos de aprobación bien posicionados; los quality gates funcionan como dispositivos de reflexión de diseño, no solo como detectores de violaciones; el proceso produce conocimiento formalizable en cada incremento.
- Hallazgos que el experimento no anticipó: los quality gates generan valor indirecto al crear checkpoints donde emerge conocimiento de diseño no automatizable; los datos reales del dominio detectan errores que sobreviven a toda la especificación formal; la IA no es un executor confiable en los pasos de coordinación humana y requiere verificación activa.
- Qué sigue abierto: validación en equipos, comparación sistemática con alternativas convencionales, sostenibilidad en proyectos de mayor escala. Estos límites son honestos y conocidos.
...
Todo lo que se enseña está respaldado por **AtaraxiaDive**, un experimento de ciclo completo diseñado explícitamente para generar evidencia empírica sobre este método. El experimento cierra con un MVP desplegado y funcional antes del inicio de la formación. Los participantes acceden a la evidencia de todo el ciclo: desde la primera especificación hasta el prototipo en producción.

El experimento produjo cinco subproyectos completados, 952 tests pasando al 100%, cero violaciones críticas de diseño sostenidas desde el segundo sprint, UAT ejecutado con datos reales de un torneo de referencia (30 atletas, 145 entradas, dispositivos físicos), y 25 hallazgos documentados que incluyen tanto lo que funcionó como lo que no. Los artefactos —código, reportes de calidad, decisiones de diseño documentadas, métricas por sprint— son accesibles para los participantes durante todo el programa.
...
El instructor no describe lo que otros hicieron. Ejecutó el experimento de principio a fin.
...
| Validación del método en equipos | La evidencia proviene de un único desarrollador. La extrapolación a equipos requiere calibración propia. |

---

*Propuesta preparada por Víctor Valotto · Abril 2026 · Versión 2.0*
