...
|----|-----|-------------|-------------------|--------|------------------------|
| B-01 | 🔴 | `resultados/domain/` importa de `competencia/domain/`. `ranking_competencia.py` y `resultados_competencia_port.py` importan `Disciplina` y `DisciplinaDescriptor` directamente de `competencia.domain`. Viola la Regla de Oro (CLAUDE.md §6): el dominio de un BC nunca importa de otro BC. **Causa raíz:** `Disciplina` vive en `competencia/domain/` pero es un tipo cross-BC que debería estar en `shared/domain/`. | `resultados/domain/aggregates/ranking_competencia.py:13-14`  `resultados/domain/ports/resultados_competencia_port.py` | `RESUELTO` | US-ADJ-2.6 (SP-ADJ-02-code): `Disciplina`, `DisciplinaDescriptor`, `UnidadMedida` movidos a `shared/domain/value_objects/`. `shared/domain/ports/event_store_port.py` creado. `competencia/domain/value_objects/` y `competencia/domain/ports/` convertidos en re-exports. Todos los imports de `resultados/domain/` actualizados a `shared.domain`. Branch: `feature/US-ADJ-2.6-disciplina-shared-domain`, commit `fb03a95`. |
| B-02 | 🔴 | `resultados/application/calcular_ranking.py` importa de `competencia/infrastructure/`. Importa `DisciplinaDescriptorAdapter` desde `competencia.infrastructure.repositories`. Double violation: (1) cross-BC import, (2) application layer → infrastructure de otro BC. | `resultados/application/commands/calcular_ranking.py:10-13` | `RESUELTO` | US-ADJ-2.6 (SP-ADJ-02-code): `DisciplinaDescriptorAdapter` creado localmente en `resultados/infrastructure/repositories/disciplina_descriptor_adapter.py`, importa desde `shared.domain.value_objects`. `resultados/application/calcular_ranking.py` actualizado. Branch: `feature/US-ADJ-2.6-disciplina-shared-domain`, commit `fb03a95`. |
| B-03 | 🟡 | `competencia/api/router.py` importa de `resultados/application/` y `resultados/infrastructure/`. El composition root de la política P-08 (CalcularRanking) vive en el router de un BC. El lugar correcto es `app.py` (el ensamblador central). | `competencia/api/router.py:70-75` | `RESUELTO` | US-ADJ-2.7 (SP-ADJ-02-code): `get_on_finalizada_callback` era código muerto (nunca usado en ningún endpoint). Eliminado de `competencia/api/router.py`. `build_on_finalizada_callback` vive en `src/app.py`. Branch: `feature/US-ADJ-2.7-composition-root-app`, commit `b4b5254`. |
| B-04 | 🟡 | `resultados/api/router.py` importa `SQLiteEventStore` directamente de `competencia/infrastructure/event_store/`. API layer de Resultados importando infra concreta de Competencia. | `resultados/api/router.py:12` | `RESUELTO` | US-ADJ-2.6 (SP-ADJ-02-code): `SQLiteEventStore` movido a `shared/infrastructure/event_store/`. `resultados/api/router.py` actualizado a `from shared.infrastructure.event_store.sqlite_event_store import SQLiteEventStore`. Branch: `feature/US-ADJ-2.6-disciplina-shared-domain`, commit `fb03a95`. |
| B-05 | 🔴 | DIP violation persiste en `competencia/api/router.py`. `EventStoreDep = Annotated[SQLiteEventStore, ...]` (línea 147) sigue usando el tipo concreto en lugar del puerto. `get_event_store()` (línea 115) retorna `SQLiteEventStore`, no `EventStorePort`. **US-ADJ-1.4 está marcada ✅ Done en BL-002 pero el código contradice ese estado.** Regresión o spec no ejecutada correctamente. | `competencia/api/router.py:115, 147` | `RESUELTO` | US-ADJ-2.8 (SP-ADJ-02-code): `get_event_store() -> EventStorePort` + `EventStoreDep = Annotated[EventStorePort, ...]`. `EventStorePort` importado desde `competencia.domain.ports.event_store_port` (re-export de `shared.domain.ports`). Branch: `feature/US-ADJ-2.8-dip-event-store`, commit `292b81f`. |
