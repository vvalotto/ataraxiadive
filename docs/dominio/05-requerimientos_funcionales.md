# AtaraxiaDive — Cuestionario de Elicitación: Requerimientos Funcionales
...
> alcance vigente usar `docs/traceability/matrix.md` y `docs/plans/sp5/PLAN-SP5.md`.
...
Este cuestionario tiene como objetivo profundizar y completar la comprensión de los requerimientos funcionales del sistema AtaraxiaDive. Las preguntas están organizadas por área funcional y surgen del análisis de la especificación inicial.
...
| RF-IN-07 | ¿Cómo se resuelve si un atleta se inscribe con datos diferentes a los que tiene la base de datos externa? | Pendiente de definición |
...
| RF-PR-02 | ¿Hay un valor mínimo o máximo permitido para los anuncios? | No se permiten valores 0 y negativos |
| RF-PR-03 | ¿Un atleta puede modificar su anuncio una vez registrado? ¿Hasta cuándo? | Implementado como upsert: el atleta puede modificar su AP mientras el torneo esté en fase INSCRIPCION_ABIERTA. La regla es enforced solo en frontend; el backend acepta el último valor registrado. |
...
| RF-EJ-04 | ¿Cuáles son los códigos de penalización posibles? ¿Siguen las reglas AIDA/CMAS u otra federación? | Implementado: tarjeta amarilla → Blanca con Penalizaciones (RP final = RP medido − N×3m). Motivos de descalificación (tarjeta roja): BKO_SUPERFICIE, BKO_SUBACUATICO, NO_PROTOCOLO, INFRACCION_TECNICA, NO_INICIO_VENTANA, SALIDA_FALSO. |
...
| RF-EJ-07 | ¿Qué información se registra en un black-out? ¿Solo el hecho o también la distancia/tiempo alcanzado? | Es black-out; se registra también la distancia alcanzada. |
...
| ID | Pregunta | Respuesta |
|---|---|---|
| RF-PM-01 | ¿Los resultados se calculan por puntos (según tablas de la federación) o por marca absoluta en cada disciplina? | Implementado con FAAS: fórmula de puntos por disciplina. Pendiente generalizar para otras federaciones. |
...
| RF-PM-03 | ¿Cómo se resuelven los empates en una disciplina? | Ocupan el mismo puesto y los mismos puntos |
...
| RF-PM-06 | ¿Los resultados deben ser publicados de alguna forma (web pública, PDF descargable)? | Se publican en la plataforma; pueden descargarse |
...
| RF-US-02 | ¿Un mismo usuario puede tener múltiples roles (por ejemplo, ser organizador y juez en distintos torneos)? | Si. Implementado: un usuario puede tener múltiples roles simultáneos (ATLETA, JUEZ, ORGANIZADOR). |
...
| ID | Pregunta | Respuesta |
|---|---|---|
| RF-NT-01 | ¿Las notificaciones son solo por email o también se contemplan notificaciones in-app o push? | Email implementado via Resend (bienvenida al registrarse). Push notifications: pendiente. |
...
> **Nota:** Este cuestionario es iterativo. Las respuestas a estas preguntas probablemente generen nuevas preguntas de seguimiento que permitirán refinar aún más la especificación. Se recomienda responder aunque sea parcialmente e indicar las dudas que surjan.
