...
class AgregarRolUsuarioCommand:
    usuario_id: UUID
    nuevo_rol: Rol


class AgregarRolUsuarioHandler:
    def __init__(self, repo: UsuarioRepositoryPort) -> None:
        self._repo = repo

    async def handle(self, cmd: AgregarRolUsuarioCommand) -> list[Rol]:
        usuario = await self._repo.find_by_id(cmd.usuario_id)
        if usuario is None:
            raise UsuarioNoEncontrado(str(cmd.usuario_id))
        usuario.agregar_rol(cmd.nuevo_rol)
        await self._repo.save(usuario)
        return list(usuario.roles)
