...
async def _tarjeta_blackout(
    store: SQLiteEventStore, cid: UUID, pid: UUID, distancia: str
) -> None:
    await AsignarTarjetaHandler(store).handle(
        AsignarTarjetaCommand(
            competencia_id=cid,
...
@pytest.mark.asyncio
async def test_get_events_orden_de_secuencia(
    store: SQLiteEventStore, client: TestClient
) -> None:
    cid, *_ = await _ejecutar_flujo_completo(store)
...
@pytest.mark.asyncio
async def test_get_events_campos_obligatorios(
    store: SQLiteEventStore, client: TestClient
) -> None:
    cid, *_ = await _ejecutar_flujo_completo(store)
...
async def test_blackout_con_distancia_en_event_store(
    ...
    blackout_events = [
        ...
        and e["data"].get("motivo") == "black-out"
    ]
